...
// Version 2: Fixed BigInt handling throughout
...
async function main() {
  ...
  try {
    ...
    console.log(`Wallet balance: ${formatTokenAmount(balance, decimals)} ${symbol}`);
    ...
    console.log(`Vesting contract balance: ${formatTokenAmount(vestingBalance, 6)} RESKA`);
    ...
    if (vestingBalance < MEDIUM_AMOUNT) {
      ...
      console.log(`Added ${formatTokenAmount(MEDIUM_AMOUNT, 6)} RESKA to vesting contract`);
      
      // Verify updated balance
      const newVestingBalance = await token.balanceOf(VESTING_ADDRESS);
      console.log(`New vesting balance: ${formatTokenAmount(newVestingBalance, 6)} RESKA`);
    }
    ...
    try {
      ...
      await tx1.wait();
      console.log(`✅ Short-term schedule created for ${formatTokenAmount(SMALL_AMOUNT, 6)} RESKA`);
    } catch (error) {
    ...
    try {
      ...
      await tx2.wait();
      console.log(`✅ Standard schedule created for ${formatTokenAmount(MEDIUM_AMOUNT, 6)} RESKA`);
    } catch (error) {
    ...
    if (beneficiarySchedulesCount > 0) {
      ...
      console.log(`- Start time: ${schedule.start.toString()}`);
      console.log(`- Cliff: ${schedule.cliff.toString()}`);
      console.log(`- Duration: ${schedule.duration.toString()}`);
      console.log(`- Slice period: ${schedule.slicePeriodSeconds.toString()}`);
      console.log(`- Amount total: ${formatTokenAmount(schedule.amountTotal, 6)} RESKA`);
      console.log(`- Amount released: ${formatTokenAmount(schedule.released, 6)} RESKA`);
      ...
      try {
        let vestedAmount;
        try {
          vestedAmount = await vesting.computeReleasableAmount(scheduleId);
        } catch (error) {
          // Fallback in case function has a different name
          try {
            vestedAmount = await vesting.getReleasableAmount(scheduleId);
          } catch (innerError) {
            console.log(`Error getting releasable amount: ${innerError.message}`);
            console.log("Checking alternative function names...");
          }
        }
        
        if (vestedAmount) {
          console.log(`Releasable amount: ${formatTokenAmount(vestedAmount, 6)} RESKA`);
          
          // If there are tokens to release, try releasing them
          if (vestedAmount > 0) {
            console.log("Attempting to release vested tokens...");
            
            // Try different release function signatures
            try {
              const releaseTx = await vesting.release(scheduleId, vestedAmount);
              await releaseTx.wait();
              console.log(`✅ Released ${formatTokenAmount(vestedAmount, 6)} RESKA to beneficiary`);
            } catch (error) {
              try {
                // Alternative signature without amount
                const releaseTx = await vesting.release(scheduleId);
                await releaseTx.wait();
                console.log(`✅ Released tokens to beneficiary`);
              } catch (innerError) {
                console.log(`Error releasing tokens: ${innerError.message}`);
              }
            }
            
            // Verify new released amount
            const updatedSchedule = await vesting.getVestingSchedule(scheduleId);
            console.log(`Updated released amount: ${formatTokenAmount(updatedSchedule.released, 6)} RESKA`);
          } else {
            console.log("No tokens available for release yet (still in cliff period)");
          }
        } else {
          console.log("Could not determine releasable amount");
        }
      } catch (error) {
        console.log(`Error computing vested amount: ${error.message}`);
      }
    ...
    try {
      ...
      console.log(`Gas for token transfer: ${transferGas.toString()}`);
    } catch (error) {
    ...
    try {
      ...
      console.log(`Gas for creating vesting schedule: ${vestingGas.toString()}`);
    } catch (error) {
