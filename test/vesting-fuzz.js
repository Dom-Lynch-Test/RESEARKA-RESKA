...
describe("RESKA Token Vesting Fuzz Tests", function() {
  ...
  async function deployVestingFixture() {
    // Get signers
    const [owner, beneficiary1, beneficiary2, beneficiary3, ...accounts] = await ethers.getSigners();
  ...
  for (let testIndex = 1; testIndex <= 5; testIndex++) {
    it(`should correctly handle vesting schedule with parameter set #${testIndex}`, async function() {
      ...
      const { token, vesting, owner, beneficiary1, startTime } = await loadFixture(deployVestingFixture);
      ...
      const amount = deterministicBigNumber(
        MIN_AMOUNT,
        MAX_AMOUNT,
        testIndex * 1000 + 4
      );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ([cliff1, duration1, cliff2, duration2, cliff3, duration3]) => {
          ...
          const { 
            token, vesting, beneficiary1, beneficiary2, beneficiary3, startTime 
          ...
          const scheduleId3 = await vesting.computeVestingScheduleIdForAddressAndIndex(beneficiary3.address, 0);
  ...
  it("should validate the specific RESKA vesting schedules", async function() {
    const { 
      token, vesting, owner, beneficiary1, beneficiary2, startTime 
    } = await loadFixture(deployVestingFixture);
  ...
  it("should handle edge cases and error conditions correctly", async function() {
    const { 
      vesting, beneficiary1, startTime 
    } = await loadFixture(deployVestingFixture);
